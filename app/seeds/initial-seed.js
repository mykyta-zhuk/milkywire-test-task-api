...
async function createImpacters(numberOfImpacters) {
  ...
  for (let index = 0; index < numberOfImpacters; index += 1) {
    impacters.push({
      name: loremIpsum({ count: 2, units: 'word' }),
      bio: loremIpsum({ count: 10 }),
      profileImage: images[index].download_url,
      status: 'ACTIVE',
    });
  }
